...
class Competitor:
    ...
    def process_assembly_shipment(self, assembly_shipment):
        """
        Grab all parts needed in assembly_shipment and store
        them in a list

        Args:
            assembly_shipment (KittingShipment): A assembly shipment from /ariac/orders
    ...
    def submit_assembly_shipment(self, assembly_stations, shipment_type):
        """ ROS service call to submit a kitting shipment
        Returns:
        bool: status of the service call
        """
        rospy.wait_for_service('/ariac/' + assembly_stations + '/submit_shipment')
        rospy.ServiceProxy('/ariac/' + assembly_stations + '/submit_shipment',
